...
def main():
    ...
    while True:
        ...
        if command_to_run == 'c':
            ...
            #print("DONE: 3 is to delete this print statement, uncomment the line below, and implement that function.")
            arm_calibration(arm_motor, touch_sensor)
        elif command_to_run == 'u':
            print("Move the arm to the up position")
            #print("DONE: 4 is to delete this print statement, uncomment the line below, and implement that function.")
            arm_up(arm_motor, touch_sensor)
        elif command_to_run == 'd':
            print("Move the arm to the down position")
            #print("DONE: 5 is to delete this print statement, uncomment the line below, and implement that function.")
            arm_down(arm_motor)
...
def arm_calibration(arm_motor, touch_sensor):
    ...
    arm_motor.run_forever(speed_sp=MAX_SPEED) #max
    while not touch_sensor.is_pressed: #is_pressed
        time.sleep(0.01)
    ...
    arm_motor.wait_while(ev3.Motor.STATE_RUNNING) #not stalled
...
def arm_up(arm_motor, touch_sensor):
    ...
    arm_motor.run_forever(speed_sp=MAX_SPEED) #max
    while not touch_sensor.is_pressed: # need the not
        time.sleep(0.01)
    arm_motor.stop()
    ev3.Sound.beep().wait()
...
def arm_down(arm_motor):
    ...
    arm_motor.run_to_rel_pos(position_sp=-14.2*360, speed_sp=MAX_SPEED) # need the correct pos
